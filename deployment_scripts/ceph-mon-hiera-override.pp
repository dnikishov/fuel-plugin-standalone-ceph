...
# Override these values in storage hash. This has to be done
# due to osnailyfacter::ceph::mon trying to manage cinder/glance
# services.
if empty(intersection($current_roles, $controller_roles)) {
  $storage_hash = {'volumes_ceph' => false,
                   'images_ceph'  => false}
} else {
  $storage_hash = {}
}
